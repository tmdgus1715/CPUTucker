...
class MemoryRegion {
 ...
 private:
  ...
  unsigned _count;  // depending on the number of CUDA streams for device memory
                    // region
};

MEMORY_REGION_TEMPLATE
MemoryRegion<MEMORY_REGION_TEMPLATE_ARGS>::MemoryRegion(void *new_ptr,
                                                        uint64_t new_size,
                                                        unsigned new_count) {
  Initialize(new_ptr, new_size, new_count);
}
MEMORY_REGION_TEMPLATE
MemoryRegion<MEMORY_REGION_TEMPLATE_ARGS>::MemoryRegion(uint64_t new_size,
                                                        unsigned new_count)
    : MemoryRegion(NULL, new_size, new_count) {}
MEMORY_REGION_TEMPLATE
MemoryRegion<MEMORY_REGION_TEMPLATE_ARGS>::MemoryRegion()
    : MemoryRegion(NULL, 0, 1) {}
MEMORY_REGION_TEMPLATE
void MemoryRegion<MEMORY_REGION_TEMPLATE_ARGS>::Initialize(void *new_ptr,
                                                           uint64_t new_size,
                                                           unsigned new_count) {
...
MEMORY_REGION_TEMPLATE
void MemoryRegion<MEMORY_REGION_TEMPLATE_ARGS>::set_memory_region(
    this_t *other) {
...
void MemoryRegion<MEMORY_REGION_TEMPLATE_ARGS>::set_size(uint64_t new_size) {
  this->_size = new_size;
  this->_aligned_size =
      common::aligned_size(new_size, 256);  // cudaAlignmentBytes: 256
}
MEMORY_REGION_TEMPLATE
