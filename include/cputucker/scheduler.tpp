...
void Scheduler<SCHEDULER_TEMPLATE_ARGS>::Schedule(tensor_t *tensor,
  ...
  if (optimizer->partition_type ==
      cputucker::enums::PartitionTypes::kNonzeroPartition) {
    this->_NonzeroBasedPartitioning(tensor, optimizer);
  } else if (optimizer->partition_type ==
             cputucker::enums::PartitionTypes::kDimensionPartition) {
...
void Scheduler<SCHEDULER_TEMPLATE_ARGS>::_NonzeroBasedPartitioning(
  ...
  this->nnz_count_per_task =
      (tensor->nnz_count + this->task_count - 1) / this->task_count;
  ...
  if (this->nnz_count_per_task > avail_nnz_count_per_task) {
    ...
    this->task_count = (tensor->nnz_count + avail_nnz_count_per_task - 1) /
...
void Scheduler<SCHEDULER_TEMPLATE_ARGS>::_DimensionBasedPartitioning(
  ...
  for (uint64_t i = 0; i < tensor->block_count; ++i) {
    ...
    if (tensor->blocks[block_id]->nnz_count > avail_nnz_count_per_stream) {
      tasks_in_block = (tensor->blocks[block_id]->nnz_count +
                        avail_nnz_count_per_stream - 1) /
                       avail_nnz_count_per_stream;
    ...
    for (t = 0; t < tasks_in_block - 1; ++t) {
      ...
      tasks[device_id].push_back(Task(block_id, nnz_count_per_stream,
                                      t * nnz_count_per_stream, stream_offset));
      ++this->task_count;

      std::cout << "[" << block_id << "] block in Device [" << device_id
                << "] - " << stream_offset << " th stream\n";
      ++global_stream_offset;
    }
    uint64_t last_nnz_count =
        tensor->blocks[block_id]->nnz_count - (t * nnz_count_per_stream);
    ...
    tasks[device_id].push_back(Task(block_id, last_nnz_count,
                                    t * nnz_count_per_stream, stream_offset));
    ++this->task_count;
