...
namespace cputucker {
  ...
  void Scheduler<SCHEDULER_TEMPLATE_ARGS>::Schedule(tensor_t *tensor, optimizer_t *optimizer) {
   if(optimizer->partition_type == cputucker::enums::PartitionTypes::kNonzeroPartition) {
      this->_NonzeroBasedPartitioning(tensor, optimizer);
    } else if(optimizer->partition_type == cputucker::enums::PartitionTypes::kDimensionPartition) {
