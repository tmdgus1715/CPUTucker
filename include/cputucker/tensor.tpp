...
Tensor<TENSOR_TEMPLATE_ARGS>::Tensor(unsigned short new_order)
  ...
  dims = cputucker::allocate<index_t>(order);
  nnz_count = 0;
  norm = 0;

  partition_dims = cputucker::allocate<index_t>(this->order);
  block_dims = cputucker::allocate<index_t>(this->order);
  blocks = NULL;
  block_count = 1;
...
void Tensor<TENSOR_TEMPLATE_ARGS>::MakeBlocks(uint64_t new_block_count,
  ...
  blocks = cputucker::allocate<block_t *>(block_count);
  index_t *block_coord = cputucker::allocate<index_t>(order);
  this->_empty_block_count = 0;
  this->_max_nnz_count_in_block = 0;

  for (uint64_t block_id = 0; block_id < block_count; ++block_id) {
    this->_BlockIDtoBlockCoord(block_id, block_coord);
    blocks[block_id] = new block_t(block_id, block_coord, order, block_dims,
                                   histogram[block_id]);
    ...
    this->_max_nnz_count_in_block =
        std::max<uint64_t>(histogram[block_id], this->_max_nnz_count_in_block);
  }
}
...
void Tensor<TENSOR_TEMPLATE_ARGS>::_RefreshDims() {
  ...
  for (unsigned short axis = 0; axis < order; ++axis) {
    ...
    this->_max_partition_dim =
        std::max<index_t>(this->_max_partition_dim, partition_dims[axis]);
    this->_max_block_dim =
        std::max<index_t>(this->_max_block_dim, block_dims[axis]);
  }
}
