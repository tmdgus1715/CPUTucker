...
__global__ void ComputingReconstructionKernel(
    ...
    const int order, const int rank, uint64_t nnz_count,
  ...
  __shared__ std::uintptr_t *sh_X_idx_addr[cputucker::constants::kMaxOrder];
  __shared__ std::uintptr_t *sh_core_idx_addr[cputucker::constants::kMaxOrder];
  __shared__ std::uintptr_t *sh_factors[cputucker::constants::kMaxOrder];
...
void ComputingReconstruction(TensorType *tensor, TensorType *core_tensor,
                             ...
                             int rank, CudaAgentType *cuda_agent,
                             SchedulerType *scheduler, int device_id) {
  ...
  std::uintptr_t **h_core_idx_addr =
      static_cast<std::uintptr_t **>(common::cuda::pinned_malloc(
          sizeof(std::uintptr_t *) * cputucker::constants::kMaxOrder));
  ...
  for (uint64_t iter = 0; iter < tasks.size(); ++iter) {
    ...
    cputucker::ComputingReconstructionKernel<index_t, value_t>
        <<<blocks_per_grid, threads_per_block, 0, streams[stream_offset]>>>(
            ...
            order, rank, avail_nnz_count, core_tensor->nnz_count);

    // For Delta
    common::cuda::d2hcpy_async(
        &error_T[block_id][nnz_offset], dev_bufs.delta.get_ptr(stream_offset),
...
#pragma omp parallel num_threads(device_count)
  {
    ...
    ComputingReconstruction(tensor, core_tensor, factor_matrices, error_T, rank,
                            cuda_agents[dev_id], scheduler, dev_id);
