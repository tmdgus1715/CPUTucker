...
TensorManager<TENSOR_MANAGER_ARGS>::TensorManager(Config *config) { this->config = config; }
...
bool TensorManager<TENSOR_MANAGER_ARGS>::ParseFromFile(const std::string &file_name, tensor_t **tensor) {
    ...
    (*tensor)->is_input_tensor = true;

    return this->_ReadData(buffer.c_str(), file_size, tensor);
}

TENSOR_MANAGER_TEMPLATE
void TensorManager<TENSOR_MANAGER_ARGS>::WriteTensorBlocksToFile(tensor_t *tensor) {
    ...
    if (!tensor_md_file.is_open()) {
        throw std::runtime_error("[ERROR] Cannot open file \"" + tensor_md_path + "\" for writing...");
    }

    std::ofstream block_md_file(block_md_path, std::ios::trunc);
    if (!block_md_file.is_open()) {
        throw std::runtime_error("[ERROR] Cannot open file \"" + block_md_path + "\" for writing...");
    }

    std::ofstream data_file(data_path, std::ios::trunc | std::ios::binary);
    if (!data_file.is_open()) {
        throw std::runtime_error("[ERROR] Cannot open file \"" + data_path + "\" for writing...");
    }
...
void TensorManager<TENSOR_MANAGER_ARGS>::_WriteBlockMetadataToFile(std::ofstream &block_md_file, block_t *block) {
    ...
    block_md_file << block->get_block_id() << " " << order << " " << block->nnz_count << " ";
...
void *TensorManager<TENSOR_MANAGER_ARGS>::ReadTensorFromFile() {
    ...
    // Additional Metadata
    tensor_md_file >> max_nnz_count_in_block >> max_block_dim >> max_partition_dim;

     uint64_t count_nnz_size = 0;
    for (ushort i = 0; i < order; ++i) {
        count_nnz_size += (block_dims[i] + 1) * sizeof(uint64_t);  // count_nnz에서 각 모드의 사이즈는 block_dim[i] + 1
    }
    
    ///test code
    fixed_data_size = (sizeof(value_t) + 2 * sizeof(index_t) * order) * max_nnz_count_in_block + count_nnz_size;
...
// block 사용 후 delete필요
TENSOR_MANAGER_TEMPLATE void *TensorManager<TENSOR_MANAGER_ARGS>::ReadBlockFromFile(uint64_t block_id) {
    const std::string block_md_path = config->getFilePath("block_metadata");
    const std::string data_path = config->getFilePath("block_data");
 ...
 */
TENSOR_MANAGER_TEMPLATE
bool TensorManager<TENSOR_MANAGER_ARGS>::_ReadData(const char *buffer, const size_t buffer_length, tensor_t **tensor) {
    int thread_id = 0;
    int thread_count = 0;
...
TENSOR_MANAGER_TEMPLATE
void TensorManager<TENSOR_MANAGER_ARGS>::WriteDeltaToFile(tensor_t *tensor, block_t *block, value_t *delta_block, int rank) {
    ...
    std::string null_str(null_size, '\0');
    delta_file.write(null_str.c_str(), null_size);
    delta_file.close();
}

TENSOR_MANAGER_TEMPLATE
void TensorManager<TENSOR_MANAGER_ARGS>::ReadDeltaFromFile(value_t *delta_block, tensor_t *tensor, block_t *block, int rank) {
    const std::string delta_path = config->getFilePath("delta");
    ...
    delta_file.read(reinterpret_cast<char *>(delta_block), curr_delta_size);
